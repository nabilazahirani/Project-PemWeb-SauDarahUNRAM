...
<div class="side-bar">
    ...
    <nav class="navbar">
        ...
        <a href="a_tentang_kami.php"><i class="fas fa-users"></i><span>Tentang Kami</span></a>.php"><i class="fas fa-users"></i><span>Tentang Kami</span></a>
    </nav>
</div>

<section class="events">
    <h1 class="heading">Jadwal Donor Darah</h1>
    <a href="a_tambah_jadwal.php" class="add-contact-btn">Tambah Jadwal</a>
    ...
    if (isset($_GET['delete'])) {
        $id = $_GET['delete'];
        $sql = "DELETE FROM jadwal_donor WHERE id_jadwal='$id'";
        if (!mysqli_query($conn, $sql)) {
            echo "<p>Error: " . mysqli_error($conn) . "</p>";
        }
    }

    $sql = "SELECT * FROM jadwal_donor ORDER BY tanggal_donor DESC";
    $result = mysqli_query($conn, $sql);

    if (mysqli_num_rows($result) > 0) {
        echo "<table>";
        echo "<tr><th>Lokasi Donor</th><th>Tanggal & Waktu</th><th>Deskripsi</th><th>Aksi</th></tr>";
        while ($row = mysqli_fetch_assoc($result)) {
            echo "<tr>";
            echo "<td>" . htmlspecialchars($row['lokasi_donor']) . "</td>";
            echo "<td>" . date("d-m-Y H:i", strtotime($row['tanggal_donor'])) . "</td>";
            echo "<td>" . nl2br(htmlspecialchars($row['deskripsi'])) . "</td>";
            echo "<td>";
            echo "<a href='a_edit_jadwal.php?id=" . $row['id_jadwal'] . "' class='edit-btn'>Edit</a> ";
            echo "<a href='?delete=" . $row['id_jadwal'] . "' class='delete-btn' onclick=\"return confirm('Yakin ingin menghapus jadwal ini?')\">Hapus</a>";
            echo "</td>";
            echo "</tr>";
        }
        echo "</table>";
    } else {
        echo "<p>Tidak ada jadwal donor yang tersedia.</p>";
    }
    ?>
</section>
